...
class BibleDataService {
  ...
  /**
   * Generates a deterministic Daily Verse from the TB translation files based on today's date
   */
  async getVerseOfTheDay(dateStr?: string): Promise<DailyVerse> {
    ...
    if (!books || books.length === 0) {
      throw new Error('Daftar kitab kosong.');
    }

    // Pick book deterministically from books list
    const bookIndex = positiveHash % books.length;
    const selectedBook = books[bookIndex];

    try {
      // Load book content directly from public/data/bible/tb/{book.file}
      const bookContent = await this.getBookContent('tb', selectedBook.id);

      if (!bookContent.chapters || bookContent.chapters.length === 0) {
        throw new Error('Tidak ada pasal dalam kitab.');
      }

      // Pick chapter deterministically
      const chapIndex = (positiveHash >> 3) % bookContent.chapters.length;
      const selectedChap = bookContent.chapters[chapIndex];

      if (!selectedChap.verses || selectedChap.verses.length === 0) {
        throw new Error('Tidak ada ayat dalam pasal.');
      }

      // Pick verse deterministically
      const verseIndex = (positiveHash >> 7) % selectedChap.verses.length;
      const selectedVerse = selectedChap.verses[verseIndex];

      return {
        text: `"${selectedVerse.text}"`,
        reference: `${selectedBook.name} ${selectedChap.chapter}:${selectedVerse.verse}`,
        translation: 'tb',
        bookId: selectedBook.id,
        bookName: selectedBook.name,
        chapter: selectedChap.chapter,
        verse: selectedVerse.verse,
      };
    } catch (err) {
      console.warn(`Gagal memuat ayat hari ini dari ${selectedBook.name}, menggunakan fallback Kejadian 1:1:`, err);
      return {
